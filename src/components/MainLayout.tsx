...
const MainLayout = ({leftSection, middleSection, rightSection}:layoutProps) => {
  ...
  const Navigation =()=> {
    ...
    const MobileNavigationBar =():React.ReactElement=> {
      const SearchBar = () => {
        const [isFocused, setIsFocused] = React.useState<boolean>(false);
        return (
          <div className={`mt-6 items-center flex w-full rounded gap-2 bg-[#f1f1f1] dark:dark:bg-inherit dark:text-white  dark:border h-[40px] ${isFocused ? 'dark:border-tertiaryBlue' : 'dark:border-white'}`}>
            <div className='h-full w-[calc(100%_-_50px)]'>
              <input type="text" placeholder='search anything e.g posts, username etc..' className='p-3 w-full h-full bg-inherit outline-none placeholder:text-[12px] dark:placeholder:text-white text-[12px]' onFocus={() => setIsFocused(true)} onBlur={() => setIsFocused(false)}/>
            </div>
            <button className={`rounded-r h-full ${isFocused ? 'bg-tertiaryBlue text-white' : ''}  flex justify-center items-center w-[50px]`}>
      ...
      return(
        <AnimatePresence>
          <motion.div className="fixed left-0 top-0 w-full h-full bg-black/[0.4] md:hidden z-[200]"
            ...
            exit={{opacity: 0}}
            >
            <motion.div className="p-3 shadow-md rounded w-[72%] fixed top-[60px] left-[20px] dark:text-white dark:bg-[#3d3d3d] bg-[white]"
